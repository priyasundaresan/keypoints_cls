...
class KeypointsAnnotator:
    ...
    def run(self, img, img_outpath):
        ...
        while True:
            ...
            # if cv2.waitKey(33) == ord('o') and len(self.clicks) == 2:
            #     self.clicks.append([-1, -1])
            #     print('No overcrossing')
            #     break
            if cv2.waitKey(33) == ord('s'):
                self.clicks = []
                print('Skipped current image')
                return None
        #self.save_heatmap(img_outpath, img, self.clicks)
        return self.clicks#[1:]

    def save_heatmap(self, img_outpath, img, points):
        point = self.clicks[0]
        # make heatmap output with point
        width, height, _ = img.shape
        gauss_sigma = 3
        gauss = gauss_2d_batch(height, width, gauss_sigma, np.array([point[0]]), np.array([point[1]]))
        gauss = gauss.reshape((height, width, 1))
        # vis_gauss(gauss)
        combined = np.append(img, gauss, axis=2)

        # overcrossing check:
        # if self.clicks[2] is None:
        #     gauss = np.zeros((width, height, 1))
        # else:
        #     point = self.clicks[2]
...
if __name__ == '__main__':
    ...
    output_dir = 'texas_granny_bowline' # Will have real_data/images and real_data/annots
    ...
    for i,f in enumerate(sorted(os.listdir(image_dir))[::-1]):
        ...
        image_outpath = os.path.join(images_output_dir, '%05d.jpg'%new_idx)
